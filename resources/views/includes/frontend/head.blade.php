...
<script async defer>
    ...
    var recaptcha = document.forms["comment_form"]["g-recaptcha-response"].value;
    if (recaptcha == "") {
...
<link  rel="icon" href="{{ URL::asset('public/images/favicon.ico') }}" type="image/x-icon"/>
...
@elseif(!empty($seo_data) && $seo_data['meta_tag']['meta_keywords'] && empty($page_data) )
<meta name="keywords" content="{{ $seo_data['meta_tag']['meta_keywords']}}">
@endif

@if(!empty($seo_data) && $seo_data['meta_tag']['meta_description'] && empty($single_product_details) && empty($page_data) )
