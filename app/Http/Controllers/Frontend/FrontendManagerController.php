...
class FrontendManagerController extends Controller
{
  ...
  public function homePageContent(){
    ...
    $data['banner']              =   Banner::latest()->where('status', 1)->get();
  ...
  public function productCategoriesSinglePageContent( $params ){
    ...
    if(isset($_GET['selected_sizes'])){
      ...
      //print_r($params);die;
    $get_category = $this->product->get_term_by_slug($params,'product_cat');
    $get_category_details = $this->product->getTermDataById($get_category->term_id);
    ...
    if(count($get_cat_product_and_breadcrumb) > 0){
      ...
      $data['product_category']   =   $get_category_details[0];
      $data['brands_data']        =   $this->product->getTermData( 'product_brands', false, null, 1 );
  ...
  /**
   *
   * Testimonial list page content
   *
   * @param null
   * @return void
   */

  public function testimonialListPageContent(){
    	$data = array();
    	$testimonials_data = array();
    	$data = $this->classCommonFunction->get_dynamic_frontend_content_data();
      	$testimonials_data  =   get_all_testimonial_data(1);

		if(count($testimonials_data) > 0){
			$data['testimonials_data'] = $testimonials_data;
		}
		else{
			$data['testimonials_data'] = array();
		}
  ...
  /**
   *
   * Download list page content
   *
   * @param null
   * @return void
   */

  public function downloadListPageContent(){
    	$data = array();
    	$download_data = array();
    	$product_obj =  new ProductsController();
    	$data = $this->classCommonFunction->get_dynamic_frontend_content_data();
      	$download_data  =   $product_obj->getTermData( 'product_brands', true, '', -1 );
		//echo '<pre>';print_r($download_data);die;
		if(count($download_data) > 0){
			$data['download_data'] = $product_obj->getTermData( 'product_brands', true, '', -1 );
		}
		else{
  ...
  public function contactWithSendEmail(){
    //if(Request::isMethod('post') && Session::token() == Request::header('X-CSRF-TOKEN')){
      $input = Request::all();
      $mailData = array();
      //print_r($input);die;
      $mail_to = 'waml.suman@gmail.com';
      $subject = 'Contact Us Mail';
      $mailData['source']           =   'quick_mail';
      $mailData['data']             =   array('_mail_to' => $mail_to, '_mail_from' => $input['customer_email'], '_subject' => $subject, '_message' => $input['message']);
  ...
  public function productSingleAffContent( $params,$aff_id ){
  ...
  public function checkoutPageContent(){
    ...
    $data['offer_product']= DB::table('offer')
                        ->where(['offer.status' => '1'])
                        ->join('products','products.id', '=', 'offer.product')->get();
      $cart_qty_by_local = 0;
      $cart_by_price = 0;
    ...
    if($this->cart->getItems()->count() > 0){
      ...
      foreach($this->cart->getItems() as $item){
        ...
        $cart_qty_by_local += $item->quantity;
        $cart_by_price     += $item->price;
    ...
    $data['razorpay_api_data'] = json_encode(get_razorpay_api_key());
    ...
    $data['local_pickup_enable']  = $local_delivery['local_delivery']['enable_option'];
    
    $data['method_title']   = $local_delivery['local_delivery']['method_title'];
    $data['fee_type']       = $local_delivery['local_delivery']['fee_type'];
  

    if($data['fee_type']=='per_product'){
      $data['delivery_fee'] = $cart_qty_by_local*$local_delivery['local_delivery']['delivery_fee'];
    }elseif($data['fee_type']=='cart_total'){
      $data['delivery_fee']= ($cart_by_price * $local_delivery['local_delivery']['delivery_fee'])/100;
    }elseif($data['fee_type']=='fixed_amount'){
  ...
  public function blogCategoriesPageContent( $params ){
    ...
    $get_category = $this->product->get_term_by_slug($params,'blog_cat');
    if(count($get_cat_post) > 0){
      $data = $this->classCommonFunction->get_dynamic_frontend_content_data();
      $data['cat_details']  =   $get_category;
  ...
  public function doActionFromCartPage(){
    ...
    elseif( Request::isMethod('post') && isset($data['update_cart']) && Session::token() == Request::get('_token')){
    ...
    elseif( Request::isMethod('post') && isset($data['checkout']) && Session::token() == Request::get('_token')){
