...
class UserAccountManageController extends Controller
{
  ...
  public function userAccountPageContent(){
    ...
    $data['login_user_details'] =  get_current_frontend_user_info();
     ...
     if(Session::get('shopist_frontend_user_role') == 'Vendor') {
      $data['offers_list']= DB::table('offer')
  ...
  /**
   * 
   * Vendor account points page content
   *
   * @param null
   * @return null
   */
  public function vendorAccountPointsPageContent(){
    $data = array();
    $data = $this->classCommonFunction->get_dynamic_frontend_content_data();
    $get_current_user_id = get_current_frontend_user_info();

    
    $data['orders_list_data'] = $this->classCommonFunction->get_point_history_by_id($get_current_user_id['unique_id']);
   ...
   /**
   * 
   * Vendor account Offers page content
   *
   * @param null
   * @return null
   */
  public function OffersForVendors(){
    $data = array();
    $data = $this->classCommonFunction->get_dynamic_frontend_content_data();
    $get_current_user_id = get_current_frontend_user_info();
  ...
  function saveUserAccountData(){
    if( Request::isMethod('post') && Session::token() == Request::Input('_token') ){
      if(Request::Input('_account_post_type') == 'address'){
        $rules = [
        ...
        if($validator->fails()){
          ...
          $address_data_ary['account_bill_first_name']              =         Request::Input('account_bill_first_name');
          $address_data_ary['account_bill_last_name']               =         Request::Input('account_bill_last_name');
          $address_data_ary['account_bill_email_address']           =         Request::Input('account_bill_email_address');
          $address_data_ary['account_bill_phone_number']            =         Request::Input('account_bill_phone_number');
          $address_data_ary['account_bill_select_country']          =         Request::Input('account_bill_select_country');
          $address_data_ary['account_bill_adddress_line_1']         =         Request::Input('account_bill_adddress_line_1');
          $address_data_ary['account_bill_adddress_line_2']         =         Request::Input('account_bill_adddress_line_2');
          ...
          $address_data_ary['account_shipping_last_name']           =         Request::Input('account_shipping_last_name');
          $address_data_ary['account_shipping_email_address']       =         Request::Input('account_shipping_email_address');
          $address_data_ary['account_shipping_phone_number']        =         Request::Input('account_shipping_phone_number');
          $address_data_ary['account_shipping_select_country']      =         Request::Input('account_shipping_select_country');
          $address_data_ary['account_shipping_adddress_line_1']     =         Request::Input('account_shipping_adddress_line_1');
          $address_data_ary['account_shipping_adddress_line_2']     =         Request::Input('account_shipping_adddress_line_2');
  ...
  public function updateFrontendUserProfile()
  {
    if( Request::isMethod('post') && Session::token() == Request::Input('_token') )
    {
      ...
      else
      {
        ...
        if(Request::Input('hf_frontend_profile_picture'))
        {
          $data['user_photo_url'] = Request::Input('hf_frontend_profile_picture');
        }
        else
        {
          $data['user_photo_url'] = '';
