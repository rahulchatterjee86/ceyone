...
class ProductsController extends Controller
{
  ...
  public function productUpdateContent( $params ){
    ...
    if(is_array($get_post)  && count($get_post) > 0){
      ...
      $data['pdetails']  =  Product::where(['id' => $product_id,])->first();
   ...
   * @return response view
   */
  public function productListContentStock( $params ){
    $data = array();
    $search_value = '';
    ...
    $data['product_all_data']  =  $this->getProductsByStock(true, $search_value, -1, $params, $is_vendor);
    $data['search_value']      =  $search_value;
    $data['settings'] = $this->option->getSettingsData();

    return view('pages.admin.product.stock-alert', $data);
  }
  ...
  public function productInventoryLog(){
    ...
    $sidebar['is_vendor_login'] = $is_vendor;
    $data['sidebar_data']       = $sidebar;
    $data['inventory_logs']     = InventoryLog::with('inventory_log')->orderBy('id','DESC')->get();
    // dd($data['inventory_logs']);

    return view('pages.admin.product.inventory_log',$data);
  }
   ...
   * @return array
   */
  public function get_term_by_slug($cat_slug,$type){
  	$get_category = DB::table('terms')
  	...
  	/*if(is_array($get_category) && count($get_category) > 0){
      $category = $this->buildTree(json_decode(json_encode($get_category), true), 0);
    }*/
    return $get_category;
  }

  public function get_categories($cat_id = 1, $cat_type){
    $categories = array();
    $term_id_where = 'orwhere';
    if(!empty($cat_id)){
		$term_id_where = 'where';
	}
    ...
    $get_categories = DB::table('terms')
                      ...
                      ->$term_id_where('terms.term_id', '=', $cat_id)
                      ->where('terms.status', '=', 1)
    ...
    if( Request::isMethod('post') && Session::token() == Request::Input('_token')){
      ...
      if($validator->fails()){
        ...
        if(Request::Input('hf_post_type') == 'add_post'){
          ...
          $post->cart_total_manage  =   Request::Input('cart_total_manage');
          $post->cart_total         =   Request::Input('cart_total_price');
          $post->discount_amount    =   Request::Input('discount_amount');
          $post->price              =   $price;
          $post->stock_qty          =   $stock_qty;
          $post->stock_availability =   $stock_availability;
          $post->type               =   Request::Input('change_product_type');
          $post->bv_points          =   Request::Input('bv_points');
          $post->appliation_point   =   Request::Input('appliation_point');
          ...
          if($post->save()){
            ...
            $instock_data = ProductExtra::where('product_id',$post->id)->where(['key_name'=>'_product_manage_stock','key_value'=>'yes'])->first();

            if(is_numeric(Request::Input('inputStockQty'))){
                InventoryLog::create([
                    'product_id'      =>  $post->id,
                    'current_stock'   =>  isset($instock_data->product_id) ? $stock_qty:'in stock',
                    'previous_stock'  =>  "in stock",
                    'stock_in_or_out' =>  $stock_qty,
        ...
        elseif (Request::Input('hf_post_type') == 'update_post'){
          $data = array(
                        ...
                        'cart_total_manage'  =>   Request::Input('cart_total_manage'),
                        'cart_total'         =>  Request::Input('cart_total_price'),
                        'discount_amount'    =>  Request::Input('discount_amount'),
                        'price'              =>  $price,
                        'stock_qty'          =>  $stock_qty,
                        'stock_availability' =>  $stock_availability,
                        'type'               =>  Request::Input('change_product_type'),
                        'bv_points'          =>  Request::Input('bv_points'),
                        'appliation_point'   =>  Request::Input('appliation_point'),
                        'image_url'          =>  $product_image
          );
          $p_qty = Product::where('id', $product_id)->first();
          ...
          if( Product::where('id', $product_id)->update($data)){
            ...
            $instock_data = ProductExtra::where('product_id',$product_id)->where(['key_name'=>'_product_manage_stock','key_value'=>'yes'])->first();
            ...
            if(is_numeric(Request::Input('inputStockQty'))){
                ...
                InventoryLog::create([
                    ...
                    'current_stock'   =>  isset($instock_data->product_id) ? $stock_qty:'in stock',
                    'previous_stock'  =>  $p_qty->stock_qty==0 ? "in stock" : $p_qty->stock_qty,
                    'stock_in_or_out' =>  $stock_qty==0 ? "stock out" : $stock_qty,
                    'created_by'      =>  $author_id
                    ]);
            }
  ...
  public function getProducts($pagination = false, $search_val = null, $status_flag = -1, $author_id, $is_vendor_login = false){
    ...
    elseif(!empty($search_val) && $search_val != '' && empty($where)){
      $get_posts_for_product = DB::table('products')
                                   ->where('products.title', 'LIKE', '%'. $search_val .'%')
                                   ->orWhere('products.stock_qty', 'LIKE', '%'. $search_val .'%')
                                   ->orWhere('products.regular_price', 'LIKE', '%'. $search_val .'%')
                                   ->orderBy('products.id', 'desc');
    }
   ...
   * @return array
   */
  public function getProductsByStock($pagination = false, $search_val = null, $status_flag = -1, $author_id, $is_vendor_login = false){
    $where = '';
    ...
    if(!empty($search_val) && $search_val != '' && !empty($where)){
      $get_posts_for_product = DB::table('products')->where($where)
                               ...
                               ->orWhere('products.title', 'LIKE', '%'. $search_val .'%')
                               ->orderBy('products.id', 'desc');
    }
    elseif(!empty($search_val) && $search_val != '' && empty($where)){
      $get_posts_for_product = DB::table('products')
  ...
  */
  ...
  public function getProductByCatSlug($cat_slug, $filter = array()){
    ...
    if(!empty($get_term) && isset($get_term->term_id)){
      ...
      //echo '<pre>';print_r($get_child_cat);die;
  ...
  public function manageExportProducts(){
    ...
    if(count($get_products) > 0){
      foreach($get_products as $rows){
        ...
        array_push($export_data, array('title' => $rows['title'], 'regular_price' => $regular_price, 'sku' => $sku ));
    ...
    $columns = array('Title','Regular Price', 'SKU');
    ...
    $callback = function() use ($export_data, $columns){
      ...
      if(count($export_data) > 0){
        foreach($export_data as $data) {
          ...
          fputcsv($file, array($data['title'], $data['regular_price'], $data['sku']));
   ...
   * @return void
   */
  public function manageStockExportProducts(){
    $export_data  = array();
    $get_products = array();

    $get_products = Product :: where('stock_qty', '<=', 10)->orderBy('id', 'DESC')->get()->toArray();
    ...
    if(count($get_products) > 0){
      foreach($get_products as $rows){
        ...
        array_push($export_data, array('title' => $rows['title'], 'regular_price' => $regular_price, 'stock_qty' => $qty, 'sku' => $sku, ) );
      }
    }

    $filename = 'stock_export_'.time().'-'.mt_rand().'.csv';
    $headers = array(
      "Content-type" => "text/csv",
      "Content-Disposition" => "attachment; filename=". $filename,
      "Pragma" => "no-cache",
    ...
    $columns = array('Title','Regular Price', 'Stock', 'SKU' );
    ...
    $callback = function() use ($export_data, $columns){
      ...
      if(count($export_data) > 0){
        foreach($export_data as $data) {
          fputcsv($file, array($data['title'], $data['regular_price'], $data['stock_qty'], $data['sku']));
        }
      }
  ...
  public function create_distributor_report(){
    $data = array();
    $data = $this->classCommonFunction->commonDataForAllPages();
    $is_vendor = is_vendor_login();
    $sidebar['is_vendor_login'] = $is_vendor;
    ...
    return view('pages.admin.product.distributor-report', $data);
  }
   ...
   * @return void
   */
  public function manageDisributorExportReport(){
    $export_data  = array();
    $get_products = array();

    $get_products = User :: where('unique_id', '!=', '')->where('user_status', '=', 1)->orderBy('id', 'DESC')->get()->toArray();
    ...
    if(count($get_products) > 0){
      foreach($get_products as $rows){
        ...
        $get_post_extras=DistributorPurchase::Where('purchased_by',$rows['id'])
                      ->whereBetween('created_at', array($start_date, $end_date.' 23:59:59'))
                      ->selectRaw("SUM(amount_with_qty) as amount")
                      ->get()
                      ->toArray();

        $points=Points::Where('credited_to',$rows['unique_id'])
                      ->whereBetween('created_at', array($start_date, $end_date.' 23:59:59'))
                      ->selectRaw("SUM(credited_points) as total_points")
                      ->get()
                      ->toArray();

        // $get_post_extras = Points::where(['purchased_by' => $rows['id']])->groupBy('order_id')->get()->toArray();

        if(count($get_post_extras) > 0){

            if($get_post_extras[0]['amount'] > 0 ){
                 $purchases = $get_post_extras[0]['amount'];
            } else {
               $purchases = 0;
            }
        ...
        if(count($points) > 0){
             if($points[0]['total_points'] > 0 ){
              $bv_points = $points[0]['total_points'];
            } else {
              $bv_points=0;
            }
        ...
        array_push($export_data, array('name' => $rows['display_name'].' '.$rows['name'], 'mobile' => $rows['mobile'], 'email' => $rows['email'], 'unique_id' => $unique_id, 'bv_points' => $bv_points, 'purchases' => $purchases));
      }
    }

    $filename = 'DistributorReport_'.date('FY').'.csv';
    $headers = array(
      "Content-type" => "text/csv",
      "Content-Disposition" => "attachment; filename=". $filename,
      "Pragma" => "no-cache",
    ...
    $columns = array('Name', 'Mobile','Email', 'Unique Id', 'BV Points', 'Purchases');
    ...
    $callback = function() use ($export_data, $columns){
      ...
      if(count($export_data) > 0){
        foreach($export_data as $data) {
          fputcsv($file, array($data['name'], $data['mobile'],$data['email'], $data['unique_id'], $data['bv_points'], $data['purchases']));
        }
      }
