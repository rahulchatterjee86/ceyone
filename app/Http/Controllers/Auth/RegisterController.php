...
class RegisterController extends Controller
{
  ...
  public function registrationProcessAll(){
    $data  = array(); 
    ...
    return view('pages.auth./all-registration')->with($get_data);
  }
  ...
  public function redirectToVendorRegistrationProcess(){
    ...
    if(isset($recaptchaData['enable_recaptcha_for_vendor_registration'])){
        $get_data['is_enable_recaptcha'] = $recaptchaData['enable_recaptcha_for_vendor_registration'];
    }
    else{
   ...
   * @return response
   */
  public function installationDataSave(Request $request){
    $is_installed = is_installed();
    $has_admin = has_admin();
        ...
        if($validator->fails()){
          ...
          if(Role::insert(
            array(
              ...
              $User->user_photo_url     =    '';
              $User->user_status        =    1;
              $User->secret_key         =    bcrypt( trim(Request::input('secret_key')) );
              ...
              if($User->save()){
                ...
                if($Roleuser->save()){
                  ...
                  $payment_method_array = array( 
                    ...
                    'razorpay'         => array('enable_option' => '', 'method_title' => Lang::get('admin.razorpay'), 'razorpay_client_id' => '', 'razorpay_secret' => '', 'razorpay_sandbox_enable_option' => 'yes', 'method_description' => $razorpay_desc),
                  ...
                  if(Option::insert(array(
                    ...
                    if($manageLanguage->save()){
                      ...
                      if(UserRolePermission::insert(array(
                          ...
                          config(['shopist.version' => '3.0.0', 'shopist.laravel' => '6.0', 'shopist.has_admin' => 1, 'shopist.is_installed' => 1]);
                          $fp = fopen(base_path() . '/config/shopist.php', 'w');
                          fwrite($fp, '<?php return ' . var_export(config('shopist'), true) . ';');
                          fclose($fp);
  ...
  public function userRegistration(){
    if( Request::isMethod('post') && Session::token() == Request::Input('_token')){
      ...
      $recaptchaData = get_recaptcha_data();
      if($recaptchaData['enable_recaptcha_for_user_registration'] == true){
      ...
      if($validator->fails()){
        ...
        $settingsData['_settings_data'] = $this->option->getSettingsData();

        if(isset($settingsData['_settings_data']['general_settings']['general_options']['default_role_slug_for_site']) && !empty($settingsData['_settings_data']['general_settings']['general_options']['default_role_slug_for_site'])){
          $User =       new User;
          $Role =       new Role;
          $Roleuser =   new RoleUser;
          ...
          if(!empty($get_role->id)){
            ...
            $User->name               =    Request::Input('user_reg_name');
            $User->email              =    Request::Input('reg_email_id');
            $User->mobile             =    Request::Input('reg_mobile_no');
            $User->password           =    bcrypt( trim(Request::Input('reg_password')) );
            $User->user_photo_url     =    '';
            $User->user_status        =    1;
            $User->secret_key         =    bcrypt( trim(Request::Input('reg_secret_key')) );
            $User->referrel_id        =    Request::Input('reg_referrel_id');
            $User->appliation_id      =   uniqid();
            // dd($User->appliation_id); 

            if($User->save()){
              $Roleuser->user_id      =    $User->id;
              $Roleuser->role_id      =    $get_role->id;

              $message="Dear ".Request::Input('user_reg_name').", You account has been Successfully registered with Ceyone. Thanks ";
           
              // $this->classCommonFunction->send_sms(Request::Input('reg_mobile_no'),$message);

              if($Roleuser->save()){
                // if($email_options['new_customer_account']['enable_disable'] == true && $this->env === 'production'){
                if($email_options['new_customer_account']['enable_disable'] == true ){
                  // $this->classGetFunction->sendCustomMail( array('source' => 'new_customer_account', 'dis_name' => Request::Input('user_reg_display_name'), 'email' => Request::Input('reg_email_id'), 'password' => Request::Input('reg_password')) );
                }
                
                Session::flash('success-message', 'Thankyou! Your request has been submitted and somebody from our team will get back to you.');
  ...
  public function vendorRegistration(){
    if( Request::isMethod('post') && Session::token() == Request::Input('_token')){
      ...
      $messages = [
        'vendor_reg_display_name.required' => 'Please fill First Name field',
        'vendor_reg_name.required' => 'Please fill Last Name field',
      ...
      $recaptchaData = get_recaptcha_data();
      if(isset($recaptchaData['enable_recaptcha_for_vendor_registration']) && $recaptchaData['enable_recaptcha_for_vendor_registration'] == true){
        $rules['g-recaptcha-response']  = 'required|captcha';
        $messages['g-recaptcha-response.required']  =  Lang::get('validation.g_recaptcha_response_required');
      }
      ...
      if($validator->fails()){
        ...
        if (!empty($get_available_roles)) {
          ...
          $referrel = substr(str_shuffle($str_result),0, 8);
          
          $User->display_name = Request::Input('vendor_reg_display_name');
          $User->name = Request::Input('vendor_reg_name');
          $User->email = Request::Input('vendor_reg_email_id');
          ...
          $User->unique_id = $referrel;
          $User->appliation_id      =   uniqid();

          $User->mobile = Request::Input('vendor_reg_phone_number');
          ...
          if ($User->save()) {
            ...
            if ($Roleuser->save()) {
              ...
              $vendor_data['profile_details'] = array('store_name' => '', 'address_line_1' => Request::Input('vendor_reg_address_line_1'), 'address_line_2' => Request::Input('vendor_reg_address_line_2'), 'city' => Request::Input('vendor_reg_city'), 'state' => Request::Input('vendor_reg_state'), 'country' => 'India', 'zip_postal_code' => Request::Input('vendor_reg_zip_code'), 'phone' => Request::Input('vendor_reg_phone_number'));
              ...
              $vendor_data['wishlists_details']=array();
              ...
              if ($Userdetails->save()) {
                ...
                if($email_options['vendor_new_account']['enable_disable'] == true ){
                  // $this->classGetFunction->sendCustomMail( array('source' => 'vendor_new_account', 'email' => Request::Input('vendor_reg_email_id')) );
                  // $this->classGetFunction->sendCustomMail( array('source' => 'vendor_new_account', 'dis_name' => Request::Input('vendor_reg_display_name'), 'email' => Request::Input('vendor_reg_email_id'), 'password' => Request::Input('vendor_reg_password')) );

                }
              
                // send welcome sms
                $message="Hi ".Request::Input('vendor_reg_display_name').", we have received your registration details. Someone from our team will get in touch with you shortly! For any further queries, you can drop a mail on info@ceyone.co.in";
                $template_id=1207162632936307618;
                $mob=Request::Input('vendor_reg_phone_number');
