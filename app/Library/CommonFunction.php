...
class CommonFunction
{
  ...
  */
   ...
   * @return void
   */
  public function add_to_cart_offer($product_id = 0, $price = null, $offer_product_name = null, $qty = 1, $variation_id = 0, $options = null, $accessToken = 0, $design_data = null){
    //check this product vendor already in cart start
    $cart_vendor_id = null;
    $new_vendor_id  = null;
    ...
    if($price > 0){
      $product_cart_line_data['price'] = $price;
    }

    if($offer_product_name){
      $product_cart_line_data['offer_product_name'] = $offer_product_name;
    }
    ...
    if( count($product_cart_line_data) >0 ){
      $_this->set_cart_data_offer( $product_cart_line_data );
    }
  }
   ...
   * @return string
   */
  public function set_cart_data_offer( $cart_data = array() ){ 
    if( count($cart_data) > 0 ){      
      $price = 0;
      $options = array();
      $variation_id = null;
      ...
      if(!empty($cart_data['offer_product_name'])){
        $pname=$cart_data['offer_product_name'];
      } else {
        $pname=$cart_data['post_title'];
      }
      
      $add_to_cart=$this->cart->add_offer_product([
        'id'            =>  $product_id,
        'added_type'    =>  "offer",
        'product_id'    =>  $cart_data['id'],
        'name'          =>  $pname,
        ...
        'order_price'   =>  get_product_price_html_by_filter($cart_data['price']),
        'img_src'       =>  $img_src,  
        'variation_id'  =>  $variation_id,
        'options'       =>  $options,
        'tax'           =>  $tax,
  ...
  */
  public function commonDataForAllPages(){
    ...
    $is_vendor_login = is_vendor_login(); 
  ...
  */
  ...
  public function send_sms($mobile_no, $msg,$temp_id){
    
      //Your authentication key
      $authKey = "115198AHpgMbHClL5757d6cd";
      ...
      //API URL
      $url="http://sms.asaptech.net/api/sendhttp.php";
   ...
   /**
   * Get Order list by product id from points table
   *
   * @param vendor_id name
   * @return array
   */
  
  public function get_point_history_by_id($vendor_id){
    $order_data = array();
    
    if(!empty($vendor_id)){
      $get_data = DB::table('points_history')
                  
                    ->where(['points_history.credited_to' => $vendor_id])
                    ->join('products', 'products.id', '=', 'points_history.product_id')  
                    ->select('points_history.order_id', 'points_history.point_per_qty',  'points_history.created_at as cdate', 'points_history.qty', 'points_history.credited_points',  'products.slug',  'products.title')
