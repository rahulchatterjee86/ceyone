...
class Cart implements CartInterface
{
     ...
     * @return \Anam\Phpcart\Collection
     */
    public function add_offer_product(Array $product)
    {
        $product_id = 0;
        $this->collection->validateItem($product);
        ...
        // If item already added, alert message
        if ( $this->has($product['id']) && ( get_product_type($product_id) == 'simple_product' || get_product_type($product_id) == 'configurable_product' || get_product_type($product_id) == 'downloadable_product' ) ) {
            $item = $this->get($product['id']);
            ...
            // return $this->updateQty($item->id, $item->quantity + $product['quantity']);
        } else if($this->is_offer_product($this->session->get($this->getCart()),'added_type','offer')){

          $st="single_product_allowed";
    ...
    public function is_offer_product($products, $field, $value)
    {
...
//        }
     ...
     * @return boolean
     */
    public function added_type($type)
    {
        $this->collection->setItems($this->session->get($this->getCart(), []));

        return $this->collection->findItem($type)? true : false;
    }
    ...
    public function cartTotalDiscountPrice(){
        $carts = $this->getItems();
          foreach($carts as $cart){
            $proudct_id = $cart->product_id;
          }
        $cart_total = Product::where('id',$proudct_id)->where('cart_total_manage',1)->first();

        if($cart_total->cart_total_manage==1){
          if($this->getTotal()>$cart_total->cart_total){
               return $this->getTotal()-($this->getTotal() * $cart_total->discount_amount / 100)??'';
            }
          }
    } 
    ...
    public function getCartTotal()
    {
      ...
      $cart_total = Product::where('id',$proudct_id)->first();
      ...
      if($this->is_coupon_applyed()){
        ...
        if($cart_total->cart_total_manage==1){
          if($this->getTotal()>$cart_total->cart_total){
              return $this->getTotal()-($this->getTotal() * $cart_total->discount_amount / 100)+$this->getShippingCost() + $this->couponPrice();
            }else{
                return ($this->getSubTotalAndTax() + $this->getShippingCost()) + $this->couponPrice();
            }
        }else{
    ...
    public function getShippingMethod()
    {
      ...
      elseif(($get_shipping['shipping_option']['enable_shipping']) && ($get_shipping['flat_rate']['enable_option'] || $get_shipping['free_shipping']['enable_option'] || $get_shipping['local_delivery']['enable_option']))
      {
        ...
        if(session('shopist_frontend_user_role') == 'Vendor'){
             if(Cart::getSubTotalAndTax() < 6000){
              return array('shipping_method' => 'local_delivery', 'shipping_cost' => 100);
             }else{
              return array('shipping_method' => 'free_shipping', 'shipping_cost' => 0);
             }
        } else {
