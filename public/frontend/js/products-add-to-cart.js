...
$(document).ready(function() 
{
  ...
  if($('.add-to-cart-bg').length>0 || $('.single-page-add-to-cart').length>0){
   ...
   //ajax request for add to cart
  if($('.add-to-cart-offer').length>0 ){
    dynamicAddToCartOffer();
  }
...
var dynamicAddToCart= function(){
  $('.add-to-cart-bg, .single-page-add-to-cart').on('click', function(e){
    ...
    dataObj['affiliation_id'] = $('#affiliation_id').val();
...
var dynamicAddToCartOffer= function(){
  $('.add-to-cart-offer').on('click', function(e){
    ...
    dataObj['product_id'] = $(this).data('id');
    dataObj['affiliation_id'] = $('#affiliation_id').val();
    dataObj['offer_product_name'] = $(this).attr('data-name');
    ...
    if($('#quantity').length>0){
     ...
     dataObj['price'] = $(this).closest('tr').attr('id'); // table row ID 
    ...
    $('#shadow-layer, .add-to-cart-loader').show();
    $.ajax({
        url: $('#hf_base_url').val() + '/ajax/add-to-cart-offer',
        type: 'POST',
        cache: false,
        datatype: 'json',
        headers: { 'X-CSRF-TOKEN' : $('meta[name="csrf-token"]').attr('content') },
        ...
        if(data && data == 'single_product_allowed'){
            $('#shadow-layer, .add-to-cart-loader').hide();
            swal("Error !","You Can not add multiple offer product in cart. please remove old item in order to apply new offer", "error");
            return false;

        } else if(data && data == 'already_added'){
            $('#shadow-layer, .add-to-cart-loader').hide();
            swal("Error !","You have already added this product to cart", "error");
            return false;
        } 
